...
\begin{enumerate}
    ...
    \item We provide extensive empirical evidence that the HAR model, despite its simplicity, is remarkably difficult to outperform with neural network approaches. Our best hybrid model achieves within 0.55\% of HAR performance.
...
\textbf{Wavelet Analysis:} We decompose the log-volatility series using discrete wavelet transform and examine the scaling of wavelet variance across scales. For a process with Hurst exponent $H$, the wavelet variance at scale $2^j$ satisfies $\text{Var}(d_j) \propto 2^{(2H+1)j}$, allowing estimation of $H$ from the slope of the log-variance versus log-scale plot.

All Hurst estimation methods are applied to \textbf{log realized volatility} series after demeaning. We report estimates from each method separately to demonstrate robustness across estimation approaches.
...
\textbf{S\&P 500 (Source Domain):} Daily realized volatility computed from high-frequency returns, covering 2018--2024 (1,467 observations). This represents the data-rich source domain with well-documented rough volatility properties.

\textbf{Bitcoin (Target Domain):} Daily realized volatility from cryptocurrency exchanges, covering 2014--2024 (17,338 observations for Hurst analysis; 5,202 observations used for model training after temporal alignment with S\&P 500 data starting from 2018). The reduced sample ensures consistent market regimes across source and target domains. Despite the longer available history, Bitcoin's higher volatility and regime changes make forecasting more challenging.
...
Table \ref{tab:hurst} presents the estimated Hurst parameters for both assets using multiple methods, and Figure \ref{fig:hurst} visualizes the wavelet analysis.
...
\begin{tabular}{lcccc}
\toprule
Asset & R/S Analysis & DFA & Variogram & Wavelet Slope \\
\midrule
S\&P 500 & 0.136 & 0.118 & 0.119 & -2.073 \\
Bitcoin & 0.109 & 0.099 & 0.097 & -2.089 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{../results/hurst_comparison.png}
\caption{Hurst parameter estimation via wavelet analysis. Top panels show log realized volatility (log RV, dimensionless) time series for S\&P 500 (1,467 observations) and Bitcoin (17,338 observations). Note: y-axis scales differ due to Bitcoin's higher volatility range. Bottom panels display the wavelet variance scaling (log scale on both axes), where the slope of the log-log regression provides the roughness estimate. Both assets exhibit slopes around -2.07 to -2.09 with high $R^2$ values (0.975 and 0.991), indicating excellent fit to the power-law scaling characteristic of rough volatility.}
\label{fig:hurst}
\end{figure}

Both assets exhibit Hurst parameters well below 0.5, confirming rough volatility characteristics. The wavelet analysis shows remarkably similar slopes for both assets (-2.073 for S\&P 500 vs -2.089 for Bitcoin), suggesting that rough volatility is a universal feature across traditional and cryptocurrency markets. Bitcoin shows slightly lower $H$ values across all methods, suggesting even rougher volatility paths, consistent with its more erratic price behavior.
...
\begin{table}[H]
\centering
\caption{Forecasting Performance on Bitcoin Test Set (n=1,279)}
\label{tab:main_results}
\begin{tabular}{lcccc}
\toprule
Model & RMSE & MAE & $R^2$ & Correlation \\
\midrule
Moving Average & 0.6191 & 0.4565 & -0.384 & 0.291 \\
LSTM (from scratch) & 0.2635 & 0.1390 & 0.749 & 0.880 \\
\textbf{Transfer LSTM} & \textbf{0.2346} & \textbf{0.1368} & \textbf{0.801} & \textbf{0.908} \\
HAR & 0.1098 & 0.0465 & 0.956 & 0.978 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{enumerate}
    \item \textbf{Transfer learning improves neural network performance:} The transfer LSTM achieves 10.99\% lower RMSE than the LSTM trained from scratch (0.2346 vs 0.2635), demonstrating that volatility patterns learned from S\&P 500 data transfer beneficially to Bitcoin forecasting.

    \item \textbf{HAR significantly outperforms neural networks:} Despite sophisticated architectures, the simple HAR model achieves substantially lower RMSE (0.1098 vs 0.2346), with $R^2 = 0.956$ compared to 0.801 for the transfer LSTM.
...
Differential LR & 0.1680 & -53.0\% & Overfitting \\
Gradual Unfreezing & 0.1550 & -41.2\% & Moderate improvement \\
Aggressive Fine-tune & 0.1448 & -31.9\% & Best unfreezing \\
\midrule
\multicolumn{4}{l}{\textit{Hybrid Architectures}} \\
\textbf{HAR-LSTM Hybrid} & \textbf{0.1104} & \textbf{-0.55\%} & \textbf{Closest to HAR} \\
Dynamic Attention & 0.1173 & -6.83\% & Over-parameterized \\
\midrule
\multicolumn{4}{l}{\textit{Error Correction (Normalized Data)$^\dagger$}} \\
Residual Boosting & -- & +0.01\% & Minimal correction \\
Regime Ensemble & -- & +0.18\% & Small improvement \\
Transfer Residual & -- & +0.00\% & No benefit \\
\bottomrule
\end{tabular}
\\[0.5em]
\footnotesize{$^\dagger$ Error correction models trained on z-score normalized data (mean=0, std=1). Percentage improvements relative to normalized HAR baseline remain valid for comparison.}
\end{table}

\subsection{Analysis of the HAR-LSTM Hybrid}

Our best-performing model, the HAR-LSTM hybrid, learns a combination weight $\alpha = 0.886$, heavily favoring HAR predictions (Figure \ref{fig:hybrid}). This reveals that:
...
The learned weight converges quickly during training, reflecting the model's discovery that HAR captures most predictable variance.
...
\textbf{2. HAR residuals are approximately noise:} When we train neural networks on HAR residuals, they learn very small corrections (standard deviation $\approx 0.005$). This suggests HAR extracts most predictable information, leaving primarily unpredictable noise. This aligns with theoretical results suggesting limited exploitable structure beyond multi-scale linear dependence when $H$ is close to zero \citep{gatheral2018volatility}.
...
\caption{Training history showing loss convergence (MSE loss) during pre-training and fine-tuning phases. The y-axis shows validation loss on a linear scale.}
...
\caption{Comparison of model predictions (log RV) against actual Bitcoin realized volatility on the test set. Closer alignment to the diagonal indicates better predictive accuracy.}
\label{fig:predictions}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{../results/hybrid_comparison.png}
\caption{HAR-LSTM hybrid model results showing the learned combination weight $\alpha$ (HAR weight) and prediction performance. Left panel: weight evolution during training. Right panel: predicted vs. actual log RV on the test set.}
\label{fig:hybrid}
\end{figure}

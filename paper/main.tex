...
\caption{Hurst parameter estimation via wavelet analysis. Top panels show the log realized volatility time series for S\&P 500 (1,467 observations) and Bitcoin (17,500+ observations). Bottom panels display the wavelet variance scaling, where the slope of the log-log regression provides the roughness estimate. Both assets exhibit slopes around -2.07 to -2.09, consistent with rough volatility ($H < 0.5$). The high $R^2$ values (0.975 and 0.991) indicate excellent fit to the power-law scaling.}
\label{fig:hurst}
\end{figure}

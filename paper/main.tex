...
\textbf{Bitcoin (Target Domain):} Daily realized volatility from cryptocurrency exchanges, covering 2014--2024 (17,338 observations for Hurst analysis; 5,202 observations used for model training after applying temporal splits). Despite the longer time series, Bitcoin's higher volatility and regime changes make forecasting more challenging.
...
\caption{Hurst parameter estimation via wavelet analysis. Top panels show the log realized volatility time series for S\&P 500 (1,467 observations) and Bitcoin (17,338 observations). Bottom panels display the wavelet variance scaling, where the slope of the log-log regression provides the roughness estimate. Both assets exhibit slopes around -2.07 to -2.09 with high $R^2$ values (0.975 and 0.991), indicating excellent fit to the power-law scaling characteristic of rough volatility.}
\label{fig:hurst}
\end{figure}
...
\caption{HAR-LSTM hybrid model results showing the learned combination weight and prediction performance.}
\label{fig:hybrid}
\end{figure}
